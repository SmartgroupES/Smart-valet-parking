...
async function initDatabase(db: D1Database) {
  ...
  try { await db.prepare('ALTER TABLE users ADD COLUMN whatsapp_apikey TEXT').run(); } catch(e) {}
    ...
    <div style="font-family: Arial, sans-serif; color: #333; max-width: 800px; margin: 0 auto; border: 1px solid #eee; padding: 20px;">
      ...
      <div style="padding: 20px;">
        ...
        <div style="background: #f0fdf4; padding: 15px; border-radius: 8px; margin-bottom: 20px; border-left: 4px solid #16a34a; display: flex; flex-direction: column; gap: 8px;">
          ...
          <p style="margin: 0;"><b>🏁 Tentativa de Culminación:</b> ${session.event_end_time || 'N/A'}</p>
        </div>
...
async function sendCallMeBotWhatsApp(phone: string, apikey: string, text: string) {
  ...
  try {
    ...
    console.log(`[WhatsApp] Enviando mensaje a ${cleanPhone} via CallMeBot...`);
    const res = await fetch(url);
    if (!res.ok) {
      console.error(`[WhatsApp] Error al enviar a ${cleanPhone}: status ${res.status}`);
    }
  } catch (e) {
    console.error('[WhatsApp] Error en sendCallMeBotWhatsApp:', e);
  }
}
...
async function checkScheduledNotifications(env: Env) {
  ...
  for (const session of sessions as any[]) {
    ...
    const staffRes = await env.DB.prepare(
      "SELECT name, role, phone, whatsapp_apikey FROM users WHERE current_session_id = ? AND is_active = 1"
    ).bind(session.id).all();
    const staff = staffRes.results || [];
    ...
    // Enviar WhatsApp individual a cada empleado citado que tenga API Key de CallMeBot
    for (const u of staff as any[]) {
      if (u.phone && u.whatsapp_apikey) {
        const msg = `🔔 *EYE STAFF: CONVOCATORIA DE EVENTO* 🔔\n\n` +
                    `Hola *${u.name.toUpperCase()}*, has sido convocado para trabajar en:\n\n` +
                    `📅 *Evento:* ${session.name}\n` +
                    `🚗 *Tipo:* ${(session.type || 'Valet').toUpperCase()}\n` +
                    `🕒 *Hora Convocatoria:* ${session.convocation_time || 'N/A'}\n` +
                    `🚀 *Hora Inicio Cita:* ${session.event_start_time || 'N/A'}\n` +
                    `📍 *Ubicación:* ${session.address || 'N/A'}\n\n` +
                    `Por favor ingresa al portal para más detalles: https://grupoeyestaff.kosak.es`;
        await sendCallMeBotWhatsApp(u.phone, u.whatsapp_apikey, msg);
      }
    }
...
app.post('/api/staff/update', async (c) => {
  ...
  const allowedFields = ['name', 'cedula', 'role', 'phone', 'email', 'address', 'sector', 'bank_name', 'bank_account', 'profile_admin', 'profile_opera', 'eye_id', 'is_active', 'pin_hash', 'emergency_contact', 'emergency_phone', 'is_allergic', 'whatsapp_apikey'];
...
app.post('/api/staff/update-bulk', async (c) => {
  ...
  const allowedFields = ['name', 'cedula', 'role', 'phone', 'email', 'birth_date', 'address', 'sector', 'bank_name', 'bank_account', 'profile_admin', 'profile_opera', 'eye_id', 'is_active', 'pin_hash', 'emergency_contact', 'emergency_phone', 'is_allergic', 'carnet_url', 'whatsapp_apikey'];
...
app.post('/api/staff/:id/notify-whatsapp', async (c) => {
  const id = c.req.param('id');
  const { message } = await c.req.json();
  if (!message) return c.json({ error: 'Mensaje vacío' }, 400);

  const user = await c.env.DB.prepare('SELECT name, phone, whatsapp_apikey FROM users WHERE id = ?').bind(id).first<any>();
  if (!user) return c.json({ error: 'Empleado no encontrado' }, 404);
  if (!user.phone || !user.whatsapp_apikey) {
    return c.json({ error: 'El empleado no tiene teléfono o API Key de WhatsApp (CallMeBot) registrada' }, 400);
  }

  await sendCallMeBotWhatsApp(user.phone, user.whatsapp_apikey, message);
  return c.json({ success: true });
});
...
app.post('/api/chat/messages', async (c) => {
  ...
  // Si es un chat grupal de evento/sesión, notificamos a todos los asignados al evento y al staff administrativo (directores y supervisores)
  if (parsedSessionId) {
    try {
      const sessionRes = await c.env.DB.prepare('SELECT name FROM sessions WHERE id = ?').bind(parsedSessionId).first<any>();
      const sessionName = sessionRes ? sessionRes.name : 'Evento';

      const senderRes = await c.env.DB.prepare('SELECT name FROM users WHERE id = ?').bind(parsedSenderId).first<any>();
      const senderName = senderRes ? senderRes.name : 'Staff';

      // Buscar todos los destinatarios que están asignados al evento o que son parte del staff administrativo
      const targetUsersRes = await c.env.DB.prepare(`
        SELECT id, name, phone, whatsapp_apikey 
        FROM users 
        WHERE (current_session_id = ? OR role IN ('director', 'supervisor')) 
          AND id != ? 
          AND is_active = 1 
          AND phone IS NOT NULL 
          AND whatsapp_apikey IS NOT NULL
      `).bind(parsedSessionId, parsedSenderId).all();
      
      const targetUsers = targetUsersRes.results || [];
      const waText = `*[VALET EYE CHAT - ${sessionName.toUpperCase()}]*\n${senderName.toUpperCase()}: ${message}`;
      
      const promises = targetUsers.map((tu: any) => sendCallMeBotWhatsApp(tu.phone, tu.whatsapp_apikey, waText));
...
app.post('/api/attendance/log', async (c) => {
  ...
  if (type === 'exit') {
    ...
    if (hasCustodyVehicles) {
      ...
      const assignedStaff = await c.env.DB.prepare(
        "SELECT id, name FROM users WHERE current_session_id = ?"
      ).bind(session_id).all();
      const staffList = assignedStaff.results || [];
