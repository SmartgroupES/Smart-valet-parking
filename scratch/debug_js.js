    ...
    window.onload = async () => {
        // 1. UI Inmediato
        updateClock();
        setInterval(updateClock, 1000); // Segundos para ver que se mueve
        initScreenSaver();
        
        console.log('EYE STAFF v2.2.2.2 Initialized');
    ...
    function exitToPortal() {
        // Ocultar todo y volver al portal
        const views = ['home-view', 'admin-view', 'staff-view', 'checkin-view', 'resumen-view', 'eventos-view'];
        views.forEach(v => {
    ...
    async function checkActiveSession() {
        ...
        const session = (res && res.sessions && res.sessions.length > 0) ? res.sessions[0] : null;
    ...
    async function showTab(tab) {
        ...
        // Bloqueo de seguridad: No permitir Recepción si no hay evento activo
        if (tab === 'checkin' && !window.activeSession) {
            toast('⚠️ DEBE INICIAR UN EVENTO EN ADMIN PARA REGISTRAR VEHÍCULOS', 'warning');
            return;
        }
        ...
        el.innerHTML = `
            <div id="home-view">
                ...
                <div class="menu-grid" style="grid-template-columns: repeat(3, 1fr); max-width: 1000px; margin: 20px auto;">
                    <div id="menu-checkin" class="menu-item ${!window.activeSession ? 'disabled' : ''}" onclick="showTab('checkin')">
                        <i>🚗</i>
                    ...
                    <div id="menu-eventos" class="menu-item ${!window.activeSession ? 'disabled' : ''}" onclick="showTab('eventos')">
                        <i>📊</i>
                        <div class="menu-item-content">
    ...
    async function renderEvents(el) {
        el.innerHTML = `
            ...
            <div class="back-nav" onclick="showTab('home')" style="margin-top:20px">← VOLVER AL MENU</div>
    ...
    function renderCustodiaList(list) {
        ...
        // Prioridad: los solicitados arriba
        ...
        container.innerHTML = sorted.map(v => `
            <div class="vehicle-card" onclick="viewVehicleDetail(${v.id})" style="border-left:4px solid ${v.status === 'pending_retrieval' ? 'var(--danger)' : 'var(--success)'}; margin-bottom:10px; cursor:pointer; position:relative; overflow:hidden;">
                ${v.status === 'pending_retrieval' ? `
                    <div style="position:absolute; top:0; right:0; background:var(--danger); color:white; font-size:0.6rem; font-weight:900; padding:4px 10px; border-bottom-left-radius:10px; animation: pulse 1s infinite;">SOLICITADA ENTREGA</div>
                ` : ''}
                <div style="display:flex; justify-content:space-between; align-items:center">
                    <div class="plate-tag">${v.plate}</div>
                    <div style="font-size:0.75rem; color:var(--muted)">ENTRADA: ${v.time.split(' ')[1]}</div>
                </div>
                <div style="margin-top:10px; font-weight:700; font-size:1rem;">${v.owner_name || 'SIN NOMBRE'}</div>
                <div style="font-size:0.8rem; color:var(--muted)">${v.brand || ''} ${v.model || ''}</div>
            </div>
        `).join('') || '<p style="text-align:center; color:var(--muted)">NO SE ENCONTRARON VEHÍCULOS</p>';
    }
    ...
    async function startNewSession() {
        ...
        try {
            const res = await apiFetch('/api/sessions/plan', { method: 'POST', body: JSON.stringify({ name }) });
            if (res) {
    ...
    async function renderCheckin(el) {
        ...
        photoData = {};
        el.innerHTML = `
            <div class="view-header"><h1 class="view-title">🚗 RECEPCIÓN</h1></div>
            ...
            <div class="grid" style="display: flex; justify-content: center;">
                <div class="stat-card" style="width: 100%; max-width: 500px; margin: 0 auto;">
                    <div class="field"><label>PLACA DEL VEHÍCULO <span style="color:var(--brand-red)">*</span></label><input type="text" id="in-plate" placeholder="EJ: ABC-123" oninput="handlePlateInput(this.value)" list="predictive-plates" required></div>
                    ...
                    <div style="display:grid; grid-template-columns: 1fr 1fr; gap:15px;">
                        <div class="field"><label>NOMBRE <span style="color:var(--brand-red)">*</span></label><input type="text" id="in-owner" placeholder="EJ: JUAN PEREZ" required></div>
                    ...
                    <div class="detail-section-title">EVIDENCIA FOTOGRÁFICA</div>
                    <div class="photo-grid">
                        <div class="photo-slot" onclick="triggerPhoto(0)" id="slot-0"><i>📷</i><span class="photo-label">FRONTAL</span></div>
                        <div class="photo-slot" onclick="triggerPhoto(1)" id="slot-1"><i>📷</i><span class="photo-label">TRASERA</span></div>
                        <div class="photo-slot" onclick="triggerPhoto(2)" id="slot-2"><i>📷</i><span class="photo-label">PILOTO</span></div>
                        <div class="photo-slot" onclick="triggerPhoto(3)" id="slot-3"><i>📷</i><span class="photo-label">COPILOTO</span></div>
                    </div>
    ...
    async function handleAIScan(input) {
        ...
        try {
            // Comprimimos MUCHO más para la IA (800px es suficiente para OCR)
            ...
            if (res) {
                if (res.plate) document.getElementById('in-plate').value = res.plate.toUpperCase();
                if (res.brand) document.getElementById('in-brand').value = `${res.brand} ${res.model || ''}`.trim();
                if (res.color) {
                    // El color lo guardamos en notas o concatenamos
                    document.getElementById('in-brand').value += ` (Color: ${res.color})`;
                }
                
                // Además, guardamos esta foto como la primera de evidencia (0 = FRONTAL)
                photoData[0] = base64;
            ...
            toast('Error en escaneo IA', 'error');
        } finally {
            hideLoading();
        }
    }
    ...
    async function renderCheckout(el) {
        el.innerHTML = `
            ...
            <div class="back-nav" onclick="showTab('home')" style="margin-top:10px">← VOLVER AL MENU</div>
    ...
    async function viewVehicleDetail(id) {
        ...
        modalBody.innerHTML = `
            ...
            <div class="grid" style="grid-template-columns:1fr 1fr; gap:10px; margin-bottom:20px">
                <div><label>NOMBRE</label><div>${v.owner_name || '—'}</div></div>
                <div><label>ESTADO</label><div>${v.status}</div></div>
            </div>
            ...
            <div style="display:flex; gap:10px; flex-wrap: wrap;">
                <button class="btn" style="flex:2; min-height:50px; font-size:0.9rem; padding: 10px;" onclick="doCheckout(${v.id}, '${v.owner_phone || ''}', '${v.plate}')">ENTREGAR VEHÍCULO Y NOTIFICAR</button>
    ...
    async function loadDetailPhotos(id) {
        ...
        if (data?.photos?.length) {
            el.innerHTML = data.photos.map((p, idx) => `
                <div class="photo-slot filled" onclick="openPhotoViewer(${JSON.stringify(data.photos.map(x => x.url)).replace(/"/g, '&quot;')}, ${idx})">
                    <img src="${API_BASE}/api/photos/${p.url}">
                </div>
            `).join('');
        } else {
            el.innerHTML = '<p style="grid-column:1/-1; color:var(--muted); text-align:center">No hay fotos</p>';
        }
    }
    ...
    async function handlePlateInput(val) {
        ...
        if (res) {
            ...
            // Si la placa coincide exactamente con una sugerencia, autocompletar
            const match = res.find(v => v.plate.toUpperCase() === val.toUpperCase());
            if (match) {
                if (match.owner_name) document.getElementById('in-owner').value = match.owner_name;
                if (match.owner_phone) document.getElementById('in-phone').value = match.owner_phone;
                if (match.brand) document.getElementById('in-brand').value = match.brand;
            }
        }
    }
    ...
    async function renderResumen(el) {
        el.innerHTML = `
            ...
            <div class="grid" style="display: flex; flex-direction: column; align-items: center; gap: 20px;">
                <div id="block-recibidos" class="stat-card" style="width: 100%; max-width: 500px; cursor: pointer; border-bottom: 5px solid var(--accent2);">
                    <div class="stat-label">RECIBIDOS (TOTAL)</div>
                    <div id="total-recibidos" class="stat-value">0</div>
                    <div id="list-recibidos" style="display: none; margin-top: 20px; text-align: left;"></div>
                </div>
            ...
            <div class="back-nav" onclick="showTab('home')" style="margin-top:30px">← VOLVER AL MENU</div>
            <div class="footer-info" style="margin-top:40px; text-align:center;">COPYRIGHT EYE STAFF 2026</div>
        `;
        
        document.getElementById('block-recibidos').onclick = () => toggleResumenList('list-recibidos');
    ...
    async function loadFullResumen() {
        ...
        const resAll = await apiFetch('/api/events/detail/RECIBIDOS?session_id=' + sid);
        if (resAll && resAll.list) {
            document.getElementById('total-recibidos').textContent = resAll.list.length;
            document.getElementById('list-recibidos').innerHTML = renderMiniList(resAll.list, 'var(--accent2)');
        }
    ...
    function renderMiniList(items, color) {
        ...
        return items.map(v => {
            const time = v.check_out_at ? new Date(v.check_out_at).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'}) : new Date(v.check_in_at).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'});
            const label = v.check_out_at ? 'SALIDA' : 'ENTRADA';
            return `
            <div style="background:var(--surface2); padding:12px; border-radius:10px; border-left:4px solid ${color}; display:flex; justify-content:space-between; align-items:center; margin-bottom:8px; border:1px solid var(--border);">
                <div>
                    <div style="font-weight:900; font-size:1.1rem; letter-spacing:1px;">${v.plate}</div>
                ...
                <div style="text-align:right">
                    ...
                    <div style="font-size:0.6rem; color:var(--muted);">${v.brand || ''}</div>
                </div>
            </div>
            `;
        }).join('');
    ...
    async function processCheckin() {
        ...
        if (res) {
            hideLoading();
            const ticketUrl = `${window.location.origin}/ticket/${res.ticket_code}`;
            const modalBody = document.getElementById('modal-body');
            modalBody.innerHTML = `
                <div style="text-align:center">
                    <div style="font-size:3rem; margin-bottom:10px">✅</div>
                    <h2 style="margin:0; color:var(--success)">REGISTRO EXITOSO</h2>
                    <div style="font-size:1.5rem; font-weight:900; margin:20px 0; letter-spacing:2px; background:var(--surface2); padding:10px; border-radius:12px; border:1px dashed var(--border)">${res.plate}</div>
                    ...
                    <div style="text-align:left; background:var(--surface2); padding:15px; border-radius:12px; margin-bottom:20px; font-size:0.9rem">
                        <div><strong>NOMBRE:</strong> ${res.owner_name}</div>
    ...
    async function renderAdmin(el) {
        ...
        el.innerHTML = `
            <div class="back-nav" onclick="showTab('home')">← VOLVER AL MENU</div>
            <div class="view-header">
                <h1 class="view-title">🔒 ADMINISTRACIÓN <small style="font-size:0.8rem; opacity:0.6;">v1.2</small></h1>
            </div>

            <!-- DOCUMENTACIÓN Y ACCESOS (TOP) -->
            ...
            <div class="stat-card" style="margin-bottom:30px; border-left:4px solid ${window.activeSession ? 'var(--success)' : 'var(--danger)'}">
                ...
                <button class="btn btn-accent" style="width:100%; margin-top:15px; height:50px; font-weight:800; ${!window.activeSession ? 'opacity:0.5; pointer-events:none;' : ''}" onclick="sendClosingReport()">📧 ENVIAR REPORTE ACTUAL (AVANCE)</button>
                <button class="btn btn-secondary" style="width:100%; margin-top:15px; height:50px; font-weight:800; border:2px solid var(--accent2);" onclick="showTab('staff')">👥 GESTIONAR PERSONAL (STAFF)</button>
            </div>
    ...
    async function closeSession() {
        if (!confirm('¿CONFIRMAR CIERRE DEL EVENTO? ESTO ENVIARÁ EL REPORTE FINAL AUTOMÁTICAMENTE.')) return;
        showLoading('CERRANDO EVENTO Y ENVIANDO REPORTE...');
        try {
            // Primero enviamos el reporte
            await apiFetch('/api/reports/send-summary', { method: 'POST' });
            // Luego cerramos la sesión
            const res = await apiFetch('/api/sessions/close', { method: 'POST' });
            if (res) {
                toast('✅ EVENTO CERRADO Y REPORTE ENVIADO', 'success');
                await checkActiveSession();
                showTab('home');
            }
        } catch (e) {
            toast('Error al cerrar sesión: ' + e.message, 'danger');
        } finally {
            hideLoading();
        }
    }
    ...
    async function renderStaff(el) {
        el.innerHTML = `
            <div class="back-nav" onclick="showTab('admin')" style="margin-bottom:30px">← VOLVER ATRÁS</div>
            ...
            <div class="stat-card" style="margin-bottom:30px; border-left:4px solid #6366f1;">
                ...
                <div class="grid" style="grid-template-columns: 1.5fr 1fr 1fr 1fr; gap:15px; align-items: flex-end;">
                    ...
                    <div class="field" style="margin-bottom:0;"><label>PIN DE ACCESO (4 DÍGITOS)</label><input type="password" id="staff-pin" maxlength="4" placeholder="...."></div>
            ...
            <div style="margin-bottom:40px;">
                ...
                <div id="staff-list-container" class="grid" style="grid-template-columns: 1fr; gap:10px;">
                    <div style="text-align:center; padding:20px;">Cargando personal...</div>
                </div>
            </div>
    ...
    async function loadStaffList() {
        ...
        fullStaffData = Array.isArray(res) ? res : (res && res.staff ? res.staff : []);
    ...
    function renderFilteredStaff(staffList) {
        ...
        if (container) {
            if (staffList.length === 0) {
                container.innerHTML = '<div style="text-align:center; padding:20px; color:var(--muted);">Sin personal registrado o no hay coincidencias</div>';
                return;
            }
            container.innerHTML = staffList.map(u => `
                <div class="stat-card" style="display:flex; justify-content:space-between; align-items:center; padding:15px; background:var(--surface2);">
                    <div>
                        <div style="font-weight:800; font-size:1.1rem;">${u.name.toUpperCase()}</div>
                        <div style="font-size:0.75rem; color:var(--accent2); text-transform:uppercase; font-weight:900; letter-spacing:1px;">${u.role}</div>
                    </div>
                    <button class="btn btn-sm btn-danger" onclick="deleteStaff(${u.id})" style="padding:5px 15px; font-size:0.7rem;">ELIMINAR</button>
                </div>
            `).join('');
        }
    }
