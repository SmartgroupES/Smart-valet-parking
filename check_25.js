    ...
    function exportPaymentFormatsExcel() {
            ...
            if (f.events && f.events.length > 0) {
                f.events.forEach(ev => {
                    ...
                    data.push({
                        ...
                        'FECHA ENVÍO': new Date(f.fecha_registro).toLocaleString().toUpperCase(),
                        'FECHA REPORTE': (f.fecha || '').toUpperCase(),
                        'NOMBRE': (f.nombre || '').toUpperCase(),
                        'CÉDULA': (f.cedula || '').toUpperCase(),
                        'CELULAR': (f.telefono_celular || '').toUpperCase(),
                        'OBSERVACIÓN': (f.observacion || '').toUpperCase(),
                        'N° EVENTO': ev.numero || '',
                        'EVENTO': (ev.evento || '').toUpperCase(),
                        'F. INICIO': (ev.fecha || '').toUpperCase(),
                        'F. FIN': (ev.fecha_fin || '-').toUpperCase(),
                        'DÍA(S)': diffDays,
                        'LUGAR': (ev.lugar || '').toUpperCase(),
                        'ACTIVIDAD': (ev.actividad || '').toUpperCase(),
        ...
        const fileName = `Formatos_transición_${yy}${mm}${dd}_${hh}${min}.xlsx`;
    ...
    window.exportConsolidatedExcel = async function() {
        if (!window.currentPaymentFormats || window.currentPaymentFormats.length === 0) {
            alert('No hay datos para exportar.');
            return;
        }
        ...
        // Fetch staff to populate dropdown
        try {
            const res = await apiFetch('/api/staff');
            if (res && res.data) {
                const select = document.getElementById('report-recipients');
                select.innerHTML = '';
                // Filter to admins or just all staff (we show all for flexibility, or filter by role='admin')
                const adminStaff = res.data.filter(s => s.role === 'admin' || s.role === 'manager' || s.role === 'director');
                const listToUse = adminStaff.length > 0 ? adminStaff : res.data;
                
                listToUse.forEach(s => {
                    const opt = document.createElement('option');
                    opt.value = s.id; // or s.phone, or s.email? The backend needs to know where to send it. We can send the ID.
                    opt.text = `${s.name} (${s.role.toUpperCase()})`;
                    select.appendChild(opt);
                });
            }
        } catch(e) {
            console.error('Error fetching staff for reports', e);
        }
    };
    
    window.generateGlobalData = function() {
        const consolidated = {};
        window.currentPaymentFormats.forEach(f => {
            const ced = f.cedula;
            if (!consolidated[ced]) {
                consolidated[ced] = {
                    'NOMBRE': (f.nombre || '').toUpperCase(),
                    'CÉDULA': (f.cedula || '').toUpperCase(),
                    'CONTACTO': (f.telefono_celular || '').toUpperCase(),
                    'TOTAL EVENTOS': 0,
                    'ADMINISTRACIÓN': 0
                };
            }
            if (f.events && f.events.length > 0) {
                ...
                consolidated[ced]['ADMINISTRACIÓN'] += f.events.reduce((s, e) => s + (e.monto || 0), 0);
            }
        });
        return Object.values(consolidated);
    };
    
    window.generateDetailedData = function() {
        const detailedData = [];
        window.currentPaymentFormats.forEach(f => {
            const totalCorrelativo = (f.events || []).reduce((s, e) => s + (e.monto || 0), 0);
            if (!f.events || f.events.length === 0) {
                detailedData.push({
                    'CORRELATIVO': f.id,
                    'NOMBRE': (f.nombre || '').toUpperCase(),
                    ...
                    'ACTIVIDAD': 'N/A',
                    'MONTO EVENTO': 0,
                    'TOTAL A PAGAR CORRELATIVO': totalCorrelativo
                });
            } else {
                f.events.forEach(ev => {
                    detailedData.push({
                        ...
                        'CÉDULA': (f.cedula || '').toUpperCase(),
                        'CONTACTO': (f.telefono_celular || '').toUpperCase(),
                        'FECHA EVENTO': new Date(ev.fecha).toLocaleDateString().toUpperCase(),
                        'LUGAR': (ev.lugar || '').toUpperCase(),
                        'ACTIVIDAD': (ev.actividad || '').toUpperCase(),
                        'MONTO EVENTO': ev.monto || 0,
                        'TOTAL A PAGAR CORRELATIVO': totalCorrelativo
                    });
                });
            }
    ...
    window.downloadReport = function(type) {
        let data = type === 'global' ? generateGlobalData() : generateDetailedData();
        const ws = XLSX.utils.json_to_sheet(data);
        const wb = XLSX.utils.book_new();
        const sheetName = type === 'global' ? "Reporte Global" : "Reporte Detallado";
        const fileName = type === 'global' ? "Consolidado_Global_EyeStaff.xlsx" : "Consolidado_Detallado_EyeStaff.xlsx";
        XLSX.utils.book_append_sheet(wb, ws, sheetName);
        XLSX.writeFile(wb, fileName);
    };

    window.sendReports = async function() {
        const select = document.getElementById('report-recipients');
        const channel = document.getElementById('report-channel').value;
        ...
        try {
            // Generate Base64 for Global
            const dataGlobal = generateGlobalData();
            const wsGlobal = XLSX.utils.json_to_sheet(dataGlobal);
            const wbGlobal = XLSX.utils.book_new();
            XLSX.utils.book_append_sheet(wbGlobal, wsGlobal, "Reporte Global");
            const base64Global = XLSX.write(wbGlobal, { type: 'base64', bookType: 'xlsx' });
            
            // Generate Base64 for Detailed
            const dataDetailed = generateDetailedData();
            const wsDetailed = XLSX.utils.json_to_sheet(dataDetailed);
            const wbDetailed = XLSX.utils.book_new();
            XLSX.utils.book_append_sheet(wbDetailed, wsDetailed, "Reporte Detallado");
            const base64Detailed = XLSX.write(wbDetailed, { type: 'base64', bookType: 'xlsx' });
            ...
            const res = await fetch('/api/admin/reports/send', {
                ...
                body: JSON.stringify({
                    ...
                    channel: channel,
                    globalBase64: base64Global,
                    detailedBase64: base64Detailed
                })
            });
            
            if (!res.ok) throw new Error('Error enviando reportes');
            
            alert('¡Reportes enviados con éxito!');
            document.getElementById('report-modal').style.display = 'none';
        } catch(e) {
            console.error('Error enviando reportes', e);
            alert('No se pudieron enviar los reportes.');
        } finally {
    ...
    window.updateEventMonto = async function(id, monto, formatId) {
            try {
                ...
                apiFetch('/api/admin/payment-formats').then(data => {
                    if(data && data.data) {
                        ...
                        if (formatId) {
                            const format = data.data.find(f => f.id == formatId);
                            if (format) {
                                const newTotal = (format.events || []).reduce((sum, ev) => sum + (ev.monto || 0), 0);
                                const td = document.getElementById('format-total-' + formatId);
                                if (td) td.innerHTML = '$' + newTotal.toFixed(2);
                            }
                        }
                    }
                });
    ...
    window.deleteFormatEvent = async function(id, formatId) {
        if (!confirm('¿Seguro que desea eliminar este evento reportado?')) return;
        try {
                const res = await fetch('/api/admin/payment-formats/events/' + id, {
                    method: 'DELETE',
                ...
                apiFetch('/api/admin/payment-formats').then(data => {
                    if(data && data.data) {
                        window.currentPaymentFormats = data.data;
                        renderAdminPayrollDashboard(document.getElementById("current-view"));
                        setTimeout(() => {
                            const detailRow = document.getElementById('pf-details-' + formatId);
