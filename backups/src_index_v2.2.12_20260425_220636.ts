...
app.post('/api/sessions/plan', async (c) => {
  ...
  const result = await c.env.DB.prepare('INSERT INTO sessions (name, type, status) VALUES (?, ?, "planning")').bind(sessionName, sessionType).run();
  return c.json({ success: true, id: result.meta.last_row_id, name: sessionName, type: sessionType, status: 'planning' });
});
...
app.post('/api/sessions/close', async (c) => {
  ...
  // VALIDACIÓN: Verificar que todo el personal asignado haya marcado salida
  const assignedStaff = await c.env.DB.prepare('SELECT id, name FROM users WHERE current_session_id = ?').bind(id).all<any>();
  const activeStaffNames: string[] = [];

  for (const staff of (assignedStaff.results || [])) {
    const lastAction = await c.env.DB.prepare(`
      SELECT type FROM staff_attendance 
      WHERE user_id = ? AND session_id = ? 
      ORDER BY timestamp DESC LIMIT 1
    `).bind(staff.id, id).first<any>();

    if (!lastAction || lastAction.type !== 'exit') {
      activeStaffNames.push(staff.name);
    }
  }

  if (activeStaffNames.length > 0) {
    return c.json({ 
      error: 'NO SE PUEDE CERRAR: Hay personal activo que no ha marcado salida.',
      active_staff: activeStaffNames 
    }, 400);
  }
...
async function sendEventClosingReport(env: Env, sessionId: number) {
  ...
  const staffActivityRes = await env.DB.prepare(`
    SELECT u.name, u.role, 
      MIN(CASE WHEN a.type = 'entry' THEN a.timestamp END) as entry_time,
      MAX(CASE WHEN a.type = 'exit' THEN a.timestamp END) as exit_time,
  ...
  `).bind(session.started_at, session.ended_at || 'CURRENT_TIMESTAMP', sessionId).all<any>();
  ...
  let html = `
    <div style="font-family: Arial, sans-serif; color: #333; max-width: 800px; margin: 0 auto; border: 1px solid #eee; padding: 20px;">
      ...
      <div style="padding: 20px;">
        ...
        <table style="width: 100%; border-collapse: collapse; font-size: 11px; margin-top: 10px;">
          ...
          ${staffActivity.map((s: any) => {
            ...
            const exit = s.exit_time ? new Date(s.exit_time) : (session.status === 'closed' ? new Date(session.ended_at) : null);
            let hours = '0.0';
            if (entry && exit) {
                const diffMs = exit.getTime() - entry.getTime();
                hours = (diffMs / (1000 * 60 * 60)).toFixed(1);
            }
            return `
            <tr>
              ...
              <td style="border: 1px solid #ddd; padding: 8px;">${entry ? entry.toLocaleTimeString() : '—'}</td>
              <td style="border: 1px solid #ddd; padding: 8px;">${exit ? exit.toLocaleTimeString() : 'ACTIVO'}</td>
              <td style="border: 1px solid #ddd; padding: 8px; font-weight: bold;">${hours}h</td>
      ...
      <div style="background: #f4f4f4; padding: 15px; text-align: center; font-size: 11px; color: #999; border-radius: 0 0 10px 10px;">
        Este reporte es generado automáticamente por EYE STAFF v2.2.7.
      </div>
    </div>
  `;
...
app.post('/api/email/start', async (c) => {
  ...
  const html = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; border: 1px solid #eee; border-radius: 20px; overflow: hidden; box-shadow: 0 4px 15px rgba(0,0,0,0.05);">
      ...
      <div style="background: #f1f5f9; padding: 20px; text-align: center; font-size: 12px; color: #94a3b8;">
        Este es un mensaje automático generado por EYE STAFF v2.2.7.
      </div>
    </div>
  `;
...
app.get('/api/staff/list', async (c) => {
  const result = await c.env.DB.prepare('SELECT id, name, role, cedula, phone, cargo, status, email FROM users ORDER BY name ASC').all();
...
app.post('/api/staff/update', async (c) => {
  ...
  const allowedFields = [
    'name', 'role', 'pin_hash', 'cedula', 'phone', 'sector', 
    'emergency_contact', 'emergency_phone', 'is_allergic', 
    'current_session_id', 'status', 'cargo', 'birth_date', 
    'email', 'address', 'bank_name', 'bank_account'
  ];
...
app.post('/api/payroll/submit', async (c) => {
  ...
  if (!data.session_ids || !Array.isArray(data.session_ids) || data.session_ids.length === 0) {
  ...
  try {
    ...
    const sessionsListHtml = data.sessions_data.map((s: any) => `
...
// ===============================
// MONITOREO Y UBICACIONES
// ===============================

// Actualizar ubicación
app.post('/api/locations/update', async (c) => {
  const { entity_id, entity_type, latitude, longitude, accuracy } = await c.req.json<LocationUpdate>();
  
  if (!entity_id || !entity_type || latitude === undefined || longitude === undefined) {
    return c.json({ error: 'Datos incompletos' }, 400);
  }

  await c.env.DB.prepare(`
    INSERT INTO locations (entity_id, entity_type, latitude, longitude, accuracy)
    VALUES (?, ?, ?, ?, ?)
  `).bind(entity_id, entity_type, latitude, longitude, accuracy || null).run();

  return c.json({ success: true });
});

// Obtener últimas ubicaciones de todos los activos/personal
app.get('/api/locations/latest', async (c) => {
  // Obtenemos la última ubicación de cada entidad en las últimas 24 horas
  const query = `
    SELECT l.*, 
      CASE 
        WHEN l.entity_type = 'staff' THEN u.name 
        WHEN l.entity_type = 'asset' THEN a.name 
      END as entity_name,
      CASE 
        WHEN l.entity_type = 'staff' THEN u.role 
        WHEN l.entity_type = 'asset' THEN a.type 
      END as entity_subinfo
    FROM locations l
    LEFT JOIN users u ON l.entity_type = 'staff' AND l.entity_id = u.id
    LEFT JOIN assets a ON l.entity_type = 'asset' AND l.entity_id = a.id
    WHERE l.id IN (
...
// Gestión de Activos (Equipos, Vehículos Propios)
app.get('/api/assets', async (c) => {
  const result = await c.env.DB.prepare('SELECT * FROM assets ORDER BY id DESC').all();
  return c.json({ assets: result.results || [] });
});

app.post('/api/assets', async (c) => {
  const { name, type, description } = await c.req.json();
  if (!name || !type) return c.json({ error: 'Nombre y Tipo requeridos' }, 400);

  const result = await c.env.DB.prepare('INSERT INTO assets (name, type, description) VALUES (?, ?, ?)')
...
app.put('/api/assets/:id', async (c) => {
  const id = c.req.param('id');
  const { name, type, description, status } = await c.req.json();
  
  await c.env.DB.prepare('UPDATE assets SET name = ?, type = ?, description = ?, status = ? WHERE id = ?')
    .bind(name, type, description, status, id)
    .run();
...
app.post('/api/admin/test-daily-report', async (c) => {
  const dateStr = new Date().toISOString().split('T')[0];
  await generateDailyReport(c.env, dateStr, "ENVÍO DE PRUEBA (HOY)");
  return c.json({ success: true, message: `Reporte de prueba enviado para la fecha ${dateStr}` });
});
...
async function handleScheduled(event: any, env: Env) {
  // El cron está configurado para las 10:00 AM
  // Queremos reportar lo ocurrido el día anterior
  const yesterday = new Date();
  yesterday.setDate(yesterday.getDate() - 1);
  const dateStr = yesterday.toISOString().split('T')[0];
  
  await generateDailyReport(env, dateStr, "REPORTE DIARIO AUTOMÁTICO");
}

async function generateDailyReport(env: Env, dateStr: string, title: string) {
  const adminEmail = env.DIRECTOR_EMAIL || 'ncarrillok@gmail.com';
  
  // 1. Buscar sesiones cerradas en esa fecha
  const sessions = await env.DB.prepare(`
    SELECT * FROM sessions 
    WHERE status = 'closed' 
    AND DATE(ended_at) = ?
  `).bind(dateStr).all<any>();

  if (!sessions.results || sessions.results.length === 0) {
    // Si no hay sesiones cerradas, enviar un correo informativo simple
    await sendEmail(env, adminEmail, `${title}: Sin actividad - ${dateStr}`, `
      <div style="font-family: Arial, sans-serif; padding: 20px; border: 1px solid #eee; border-radius: 10px;">
        <h2 style="color: #ef4444;">${title}</h2>
        <p>No se registraron eventos cerrados el día <b>${dateStr}</b>.</p>
        <hr>
        <p style="font-size: 12px; color: #999;">EYE STAFF v2.2.7</p>
      </div>
    `);
    return;
  }
  ...
  let fullCsv = "EVENTO;PLACA;MARCA;MODELO;COLOR;ENTRADA;SALIDA;TIEMPO_MIN;COBRO\n";

  for (const session of sessions.results) {
    const vehiclesRes = await env.DB.prepare('SELECT * FROM vehicles WHERE session_id = ?').bind(session.id).all<any>();
    const vehicles = vehiclesRes.results || [];
    ...
    vehicles.forEach((v: any) => {
      ...
      const exit = v.check_out_at ? new Date(v.check_out_at) : null;
      const diff = exit ? Math.round((exit.getTime() - entry.getTime()) / 60000) : 0;
      sessionFees += (v.fee_amount || 0);
      
      fullCsv += `${session.name};${v.plate};${v.brand || ''};${v.model || ''};${v.color || ''};${v.check_in_at};${v.check_out_at || ''};${diff};${v.fee_amount || 0}\n`;
      
      vehicleRows += `
        <tr>
          <td style="border: 1px solid #ddd; padding: 5px;">${v.plate}</td>
          <td style="border: 1px solid #ddd; padding: 5px;">${v.brand || ''}</td>
          <td style="border: 1px solid #ddd; padding: 5px;">${v.status}</td>
          <td style="border: 1px solid #ddd; padding: 5px;">$${v.fee_amount || 0}</td>
        </tr>
      `;
    });
    ...
    sessionSummaries += `
      <div style="margin-top: 20px; padding: 15px; background: #f8fafc; border-radius: 8px; border-left: 4px solid #ef4444;">
        <h4 style="margin: 0; color: #0f172a;">${session.name}</h4>
        <p style="margin: 5px 0; font-size: 13px;">Vehículos: <b>${vehicles.length}</b> | Recaudación: <b>$${sessionFees}</b></p>
        <table style="width: 100%; border-collapse: collapse; font-size: 11px; margin-top: 10px;">
          <tr style="background: #e2e8f0;">
            <th style="padding: 5px; text-align: left;">Placa</th>
            <th style="padding: 5px; text-align: left;">Marca</th>
            <th style="padding: 5px; text-align: left;">Estado</th>
            <th style="padding: 5px; text-align: left;">Cobro</th>
          </tr>
          ${vehicleRows}
        </table>
      </div>
  ...
  // 2. Generar Resumen Ejecutivo con AI
  let executiveSummary = "No se pudo generar el resumen ejecutivo.";
  try {
    const aiPrompt = `Resume ejecutivamente la operación de valet parking del día ${dateStr}. 
    Se cerraron ${sessions.results.length} eventos. 
    Total de vehículos atendidos: ${totalVehicles}. 
    Recaudación total: $${totalFees}.
    Eventos destacados: ${sessions.results.map((s: any) => s.name).join(', ')}.
    Escribe el resumen en español, en un tono profesional y directo. Máximo 3 párrafos.`;

    const aiRes = await env.AI.run('@cf/meta/llama-3-8b-instruct', {
      messages: [
        { role: 'system', content: 'Eres un director de operaciones experto en logística de eventos y valet parking.' },
        { role: 'user', content: aiPrompt }
      ]
    });
    executiveSummary = aiRes.response || aiRes.text || "Resumen generado con éxito.";
  } catch (e) {
    console.error("AI Error:", e);
  }

  // 3. Formatear HTML Final
  const html = `
    <div style="font-family: Arial, sans-serif; color: #333; max-width: 800px; margin: 0 auto; border: 1px solid #eee; border-radius: 12px; overflow: hidden;">
      <div style="background: #ef4444; color: white; padding: 30px; text-align: center;">
        ...
        <p style="margin: 5px 0 0 0; opacity: 0.9;">Operaciones del día: ${dateStr}</p>
      </div>

      <div style="padding: 30px;">
        <div style="display: flex; gap: 20px; margin-bottom: 30px; text-align: center;">
          <div style="flex: 1; padding: 20px; background: #fff1f2; border-radius: 12px;">
            <div style="font-size: 12px; color: #e11d48; text-transform: uppercase; font-weight: bold;">Vehículos</div>
            <div style="font-size: 32px; font-weight: 800; color: #9f1239;">${totalVehicles}</div>
          </div>
          <div style="flex: 1; padding: 20px; background: #f0fdf4; border-radius: 12px;">
            <div style="font-size: 12px; color: #16a34a; text-transform: uppercase; font-weight: bold;">Recaudación</div>
            <div style="font-size: 32px; font-weight: 800; color: #14532d;">$${totalFees}</div>
          </div>
          <div style="flex: 1; padding: 20px; background: #eff6ff; border-radius: 12px;">
            <div style="font-size: 12px; color: #2563eb; text-transform: uppercase; font-weight: bold;">Eventos</div>
            <div style="font-size: 32px; font-weight: 800; color: #1e3a8a;">${sessions.results.length}</div>
          </div>
        </div>

        <h3 style="color: #0f172a; border-bottom: 2px solid #ef4444; padding-bottom: 8px;">RESUMEN EJECUTIVO</h3>
        <div style="line-height: 1.6; color: #475569; background: #f8fafc; padding: 20px; border-radius: 12px; margin-bottom: 30px; white-space: pre-wrap;">
          ${executiveSummary}
        </div>

        <h3 style="color: #0f172a; border-bottom: 2px solid #ef4444; padding-bottom: 8px;">DETALLE POR EVENTO</h3>
        ${sessionSummaries}
      </div>

      <div style="background: #f1f5f9; padding: 20px; text-align: center; font-size: 12px; color: #94a3b8;">
        Este reporte consolidado es generado automáticamente por EYE STAFF Intelligence.
      </div>
    </div>
  `;
  ...
  const attachments = [
    {
      filename: `Reporte_Consolidado_${dateStr}.csv`,
      content: btoa(fullCsv)
    }
  ];

  await sendEmail(env, adminEmail, `${title}: ${dateStr}`, html, attachments);
}
