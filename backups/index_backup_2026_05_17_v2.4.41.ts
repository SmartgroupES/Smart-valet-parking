...
app.post('/api/sessions/close', async (c) => {
  ...
  const dbUser = await c.env.DB.prepare('SELECT pin_hash, name FROM users WHERE id = ?').bind(currentUserId).first<any>();
  if (!dbUser || dbUser.pin_hash !== pin.toString().trim()) {
...
async function generateBirthdayPDF(guys: any[], monthName: string) {
  ...
  page.drawText('FECHA NACIMIENTO', { x: 260, y: currentY - 2, size: 10, font: boldFont, color: rgb(0.06, 0.09, 0.16) });
  page.drawText('DIA', { x: 400, y: currentY - 2, size: 10, font: boldFont, color: rgb(0.06, 0.09, 0.16) });
  ...
  for (const guy of guys) {
    ...
    page.drawText(birthDate, { x: 260, y: currentY + 3, size: 9, font: font, color: rgb(0.1, 0.1, 0.1) });
    page.drawText(day, { x: 400, y: currentY + 3, size: 9, font: boldFont, color: rgb(0.388, 0.4, 0.945) });
...
async function sendMonthlyBirthdayReport(env: Env, forceTest: boolean = false) {
  ...
  // Ordenar de mas antiguo a mas reciente (absoluto: ano-mes-dia)
  ...
  for (const guy of birthdayGuys) {
    ...
    listHtml += `
      <tr style="border-bottom:1px solid #f1f5f9;">
        ...
        <td style="padding:12px 15px; color:#6366f1; font-weight:900; text-align:center;">Dia ${day}</td>
        <td style="padding:12px 15px; color:#475569; text-align:center;">${guy.birth_date}</td>
    ...
    <div style="font-family:sans-serif; max-width:600px; margin:auto; border:1px solid #eee; border-radius:15px; overflow:hidden; border-top:6px solid #6366f1;">
      ...
      <div style="padding:30px; background:#fff;">
        <div style="background:#f8fafc; padding:20px; border-radius:12px; margin-bottom:25px; border-left:4px solid #6366f1;">
          ...
          <p style="margin:0 0 0; color:#475569;">A continuacion se detalla el listado de personal que celebra su cumpleanos el proximo mes, ordenado de mas antiguo a mas reciente.</p>
        </div>
        ...
        <table style="width:100%; border-collapse:collapse; font-size:0.9rem; margin-bottom:25px;">
          <thead>
            <tr style="background:#f8fafc; border-bottom:2px solid #e2e8f0; text-align:left;">
              ...
              <th style="padding:10px 15px; color:#475569; text-align:center;">Dia</th>
              <th style="padding:10px 15px; color:#475569; text-align:center;">Fecha</th>
...
app.post('/api/auth/change-pin', async (c) => {
  ...
  if (!existing || existing.pin_hash !== current_pin) {
    return c.json({ error: 'El PIN actual es incorrecto' }, 400);
  }

  await c.env.DB.prepare('UPDATE users SET pin_hash = ? WHERE id = ?').bind(new_pin, user.id).run();
...
app.post('/api/admin/verify-pin-and-query', async (c) => {
  ...
  const adminUser: any = await c.env.DB.prepare('SELECT name, pin_hash FROM users WHERE id = ?').bind(current.id).first();
  if (!adminUser) {
    return c.json({ error: 'Usuario no encontrado' }, 404);
  }

  if (adminUser.pin_hash !== admin_pin.trim()) {
  ...
  if (force) {
    try {
      ...
      await c.env.DB.batch([
        ...
        c.env.DB.prepare('DELETE FROM messages WHERE from_id = ?').bind(id),
        c.env.DB.prepare('DELETE FROM users WHERE id = ?').bind(id)
      ]);
      return c.json({ success: true, message: 'Registro y TODO su historial eliminados definitivamente (v5)' });
    } catch (e: any) {
