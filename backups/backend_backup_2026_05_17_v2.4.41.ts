...
app.get('/api/chat/messages', async (c) => {
  ...
  if (session_id) {
    ...
    binds.push(sender_id, recipient_id, recipient_id, sender_id);
  } else {
...
app.post('/api/chat/messages', async (c) => {
  ...
  await c.env.DB.prepare(
    ...
    sender_id, 
    recipient_id ? parseInt(recipient_id.toString()) : null, 
    session_id ? parseInt(session_id.toString()) : null, 
    message
  ).run();
...
app.get('/api/admin/online-now', async (c) => {
  const query = `
    ...
    WHERE ws.is_active = 1
      AND datetime(ws.last_activity_at) >= datetime('now', '-15 minutes')
    GROUP BY u.id
