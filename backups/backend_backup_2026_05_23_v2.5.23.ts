...
app.post('/api/presupuestos', async (c) => {
  try {
    ...
    let seq = 1;
    const seqData = await c.env.DB.prepare('SELECT seq FROM budget_seq WHERE year = ?').bind(currentYear).first();
    if (seqData) {
      seq = (seqData.seq as number) + 1;
      await c.env.DB.prepare('UPDATE budget_seq SET seq = ? WHERE year = ?').bind(seq, currentYear).run();
    } else {
      await c.env.DB.prepare('INSERT INTO budget_seq (year, seq) VALUES (?, 1)').bind(currentYear).run();
    }
    
    const correlativo = String(seq).padStart(3, '0');
