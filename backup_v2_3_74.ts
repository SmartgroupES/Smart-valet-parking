...
async function checkScheduledNotifications(env: Env) {
  ...
  const now = new Date();
  const offset = 2; // Basado en el local time del metadata (+02:00)
    ...
    const html = `
      <div style="font-family: 'Outfit', sans-serif; background: #0b0f19; color: white; padding: 30px; border-radius: 20px; border: 1px solid #1e253c; max-width: 600px; margin: 0 auto;">
        ...
        <div style="background: #161b2c; padding: 25px; border-radius: 15px; border: 1px solid rgba(255,255,255,0.05);">
          ...
          <p style="font-size: 1.1rem;">Hola, ha llegado la hora de la convocatoria para el siguiente evento:</p>
          
          <table style="width: 100%; border-collapse: collapse; margin-top: 20px;">
            <tr>
              <td style="color: #94a3b8; padding: 8px 0; font-size: 0.9rem;">EVENTO:</td>
              <td style="color: white; padding: 8px 0; font-weight: bold; text-align: right;">${session.name}</td>
            </tr>
            <tr>
              <td style="color: #94a3b8; padding: 8px 0; font-size: 0.9rem;">HORA:</td>
              <td style="color: #ef4444; padding: 8px 0; font-weight: bold; text-align: right;">${session.convocation_time}</td>
            </tr>
            <tr>
              <td style="color: #94a3b8; padding: 8px 0; font-size: 0.9rem;">DIRECCIÓN:</td>
              <td style="color: white; padding: 8px 0; font-size: 0.8rem; text-align: right;">${session.address || 'N/A'}</td>
            </tr>
          </table>
        </div>
...
app.post('/api/staff/login', async (c) => {
  try {
    ...
    // Buscamos ignorando espacios para máxima flexibilidad
    const searchName = inputName.replace(/\s+/g, '');
    const dbUser = await c.env.DB.prepare(`
      SELECT * FROM users 
      WHERE (REPLACE(LOWER(name), ' ', '') LIKE ? OR LOWER(name) LIKE ?) 
      AND is_active = 1
    `)
      .bind(`%${searchName}%`, `%${inputName}%`)
      .first();
    ...
    if (dbUser && dbUser.pin_hash === lowerPass) {
        ...
        const sessionResult = await c.env.DB.prepare('INSERT INTO web_sessions (user_id, device, ip, is_active) VALUES (?, ?, ?, 1)')
...
app.post('/api/staff', async (c) => {
  ...
  const { name, pin_hash, role, cedula, phone, address, sector, bank_name, bank_account, carnet, profile_admin, profile_opera, eye_id, email, emergency_contact, emergency_phone, is_allergic } = await c.req.json();
  ...
  await c.env.DB.prepare('INSERT INTO users (name, pin_hash, role, cedula, phone, address, sector, bank_name, bank_account, carnet_url, profile_admin, profile_opera, eye_id, email, emergency_contact, emergency_phone, is_allergic) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)')
    .bind(cleanName, pin_hash, mappedRole, cedula || null, phone || null, address || null, sector || null, bank_name || null, bank_account || null, carnetKey, profile_admin || null, profile_opera || null, eye_id || null, email || null, emergency_contact || null, emergency_phone || null, is_allergic || null)
    .run();
...
app.post('/api/staff/import', async (c) => {
  ...
  for (let line of lines.slice(1)) { // Skip header
    ...
    // Nuevo mapeo según orden de imagen:
    // 0:Item, 1:Status, 2:Nombre, 3:Cedula, 4:Email, 5:EYE_ID, 6:P_Admin, 7:P_Opera, 8:Tel, 9:Dir, 10:Bank, 11:Account, 12:Fam, 13:TelFam, 14:Alergias
    const pin = '1234'; 
    const name = cols[2] || '';
    const cedula = cols[3] || '';
    const email = cols[4] || '';
    ...
    const phone = cols[8] || '';
    const sector = cols[9] || '';
    const bank_name = cols[10] || '';
    const bank_account = cols[11] || '';
    const e_contact = cols[12] || '';
    const e_phone = cols[13] || '';
    const allergies = cols[14] || '';
    ...
    await c.env.DB.prepare(`
      INSERT INTO users (
        name, pin_hash, role, cedula, email, phone, sector, 
        bank_name, bank_account, emergency_contact, emergency_phone, 
        is_allergic, profile_admin, profile_opera, eye_id
      ) 
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      ON CONFLICT(name) DO UPDATE SET 
        ...
        eye_id = excluded.eye_id
    `).bind(
      cleanName, pin, role, cedula, email, phone, sector, 
      bank_name, bank_account, e_contact, e_phone, 
      allergies, pAdmin, pOpera, eye_id
    ).run().catch(e => console.error('Import row error:', e));
