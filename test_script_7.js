...
window.enviarBackup = async function(channel) {
    if (!confirm("¿Está seguro que desea generar y enviar el backup completo del sistema? Esto recopilará la base de datos entera.")) return;
...
window.enviarReporteCierreDiario = async function(preview = false) {
    if (!preview && !confirm("¿Está seguro que desea compilar y enviar el Cierre Diario Global a los directores suscritos?")) return;
    showLoading(preview ? 'GENERANDO VISTA PREVIA...' : 'COMPILANDO CIERRE DIARIO Y GENERANDO PDFs...');
    try {
        const res = await apiFetch('/api/admin/cierre-diario-data');
        if (!res || !res.data) throw new Error("No se pudo obtener la información del servidor");
        
        const sessions = res.data;
        const { jsPDF } = window.jspdf;
        
        let totalEvents = sessions.length;
        let totalStaff = 0;
        let totalVehicles = 0;
        let attachments = [];
        let htmlRows = '';
        ...
        for (const s of sessions) {
            ...
            let elapsed = '0H 0M';
            if (ev.started_at) {
                const d = new Date(ev.started_at.replace(/-/g, '/')); // safe date parsing
                if (ev.event_start_time) {
                    const [h, m] = ev.event_start_time.split(':');
                    d.setHours(parseInt(h) || 0, parseInt(m) || 0, 0, 0);
                }
                const diffMs = new Date() - d;
                if (diffMs > 0) {
                    elapsed = `${Math.floor(diffMs / 3600000)}H ${Math.floor((diffMs % 3600000) / 60000)}M`;
                }
            }
            
            const startT = ev.event_start_time ? ev.event_start_time : new Date(ev.started_at.replace(/-/g, '/')).toLocaleTimeString('es-ES', {hour:'2-digit', minute:'2-digit'});
            const vehOut = veh.filter(v => v.status === 'delivered').length;

            htmlRows += `
                <tr style="border-bottom: 1px solid #eee;">
                    <td style="padding: 10px; font-weight: bold;">${ev.name.toUpperCase()}</td>
                    <td style="padding: 10px; text-align: center;">${ev.type}</td>
                    <td style="padding: 10px; text-align: center;">${startT}</td>
                    <td style="padding: 10px; text-align: center;">${elapsed}</td>
                    <td style="padding: 10px; text-align: center; color: #10b981;">👤 ${staff.length}</td>
                    <td style="padding: 10px; text-align: center;">${veh.length} IN / ${vehOut} OUT</td>
                </tr>
            `;
            ...
            const doc = new jsPDF();
            doc.setFontSize(16);
            doc.text("REPORTE DETALLADO DE EVENTO (CIERRE)", 14, 20);
            
            doc.setFontSize(10);
            doc.text(`Evento: ${ev.name.toUpperCase()}`, 14, 30);
            doc.text(`Tipo: ${ev.type}`, 14, 35);
            doc.text(`Inicio: ${startT} | Transcurrido: ${elapsed}`, 14, 40);
            doc.text(`Comidas Reportadas -> Desayunos: ${ev.desayunos || 0} | Almuerzos: ${ev.almuerzos || 0} | Cenas: ${ev.cenas || 0}`, 14, 45);
            
            doc.autoTable({
                startY: 50,
                head: [['Empleado', 'Perfil', 'Acción', 'Hora']],
                body: staff.map(st => [st.name, st.role, st.type, new Date(st.timestamp.replace(/-/g, '/')).toLocaleTimeString()]),
                theme: 'grid',
            ...
            if (veh.length > 0) {
                doc.autoTable({
                    ...
                    head: [['Placa', 'Marca/Modelo', 'Estado', 'Ingreso', 'Salida']],
                    body: veh.map(v => [v.plate, (v.brand||'')+' '+(v.model||''), v.status, new Date(v.check_in_at.replace(/-/g, '/')).toLocaleTimeString(), v.check_out_at ? new Date(v.check_out_at.replace(/-/g, '/')).toLocaleTimeString() : '']),
                    theme: 'grid',
            ...
            attachments.push({
                filename: `Reporte_${ev.name.replace(/\s+/g, '_')}.pdf`,
                content: pdfBase64
            });
        }
        ...
        const htmlContent = `
            <div style="font-family: Arial, sans-serif; max-width: 700px; margin: auto; border: 1px solid #e5e7eb; border-radius: 12px; overflow: hidden;">
                <div style="background-color: #0f172a; padding: 20px; text-align: center; border-bottom: 4px solid #ef4444;">
                    <h1 style="color: #fff; margin: 0; font-size: 20px;">REPORTE DE CIERRE DIARIO</h1>
                    <p style="color: #94a3b8; font-size: 14px; margin-top: 5px;">${new Date().toLocaleString()}</p>
                </div>
                <div style="padding: 20px; background-color: #f8fafc;">
                    <div style="display: flex; gap:10px; margin-bottom:20px; background:#fff; padding:15px; border-radius:8px; text-align:center; border: 1px solid #e2e8f0;">
                        <div style="flex:1;">
                            <div style="font-size:24px; font-weight:bold; color:#10b981;">${totalEvents}</div>
                            <small style="color:#64748b; font-weight:bold;">EVENTOS ACTIVOS</small>
                        </div>
                        <div style="flex:1; border-left:1px solid #e2e8f0;">
                            <div style="font-size:24px; font-weight:bold; color:#3b82f6;">${totalStaff}</div>
                            <small style="color:#64748b; font-weight:bold;">PERSONAL EN TURNO</small>
                        </div>
                        <div style="flex:1; border-left:1px solid #e2e8f0;">
                            <div style="font-size:24px; font-weight:bold; color:#ef4444;">${totalVehicles}</div>
                            <small style="color:#64748b; font-weight:bold;">VEHÍCULOS IN</small>
                        </div>
                    </div>
                    <table style="width: 100%; border-collapse: collapse; font-size: 12px; background: #fff; border: 1px solid #e2e8f0;">
                        <thead>
                            <tr style="background-color: #f1f5f9; text-transform: uppercase;">
                                ...
                                <th style="padding: 10px;">Asistentes</th>
                            </tr>
                        </thead>
                        <tbody>
                            ${htmlRows || '<tr><td colspan="6" style="text-align:center; padding:10px;">No hay eventos activos</td></tr>'}
                        </tbody>
                    </table>
                    <p style="font-size:11px; color:#64748b; text-align:center; margin-top:20px;">
                        Los anexos detallados en formato PDF de cada evento se encuentran adjuntos a este correo.
                    </p>
                </div>
            </div>
        `;
        ...
        if (preview) {
            ...
            if (!win) { alert('⚠️ Permita las ventanas emergentes (pop-ups) para ver la vista previa.'); return; }
            win.document.write(htmlContent);
            win.document.close();
            toast('✅ VISTA PREVIA ABIERTA', 'success');
            return;
        }

        showLoading('ENVIANDO EMAILS CON ADJUNTOS...');
        const sendRes = await apiFetch('/api/admin/send-cierre-diario', {
            method: 'POST',
            body: JSON.stringify({ html: htmlContent, attachments })
        });

        hideLoading();
        if (sendRes && sendRes.success) {
            toast(`✅ Cierre enviado a ${sendRes.emailsCount} directores suscritos`, 'success');
        } else {
            toast('❌ Error al enviar: ' + (sendRes ? sendRes.error : 'Desconocido'), 'error');
        }
