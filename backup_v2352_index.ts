...
app.post('/api/staff', async (c) => {
  ...
  const { name, pin_hash, role, cedula, phone, sector, bank_name, bank_account, carnet, profile_admin, profile_opera, eye_id } = await c.req.json();
  ...
  await c.env.DB.prepare('INSERT INTO users (name, pin_hash, role, cedula, phone, sector, bank_name, bank_account, carnet_url, profile_admin, profile_opera, eye_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)')
    .bind(cleanName, pin_hash, mappedRole, cedula || null, phone || null, sector || null, bank_name || null, bank_account || null, carnetKey, profile_admin || null, profile_opera || null, eye_id || null)
    .run();
...
app.post('/api/staff/update', async (c) => {
  ...
  const allowedFields = ['name', 'role', 'pin_hash', 'cedula', 'phone', 'sector', 'emergency_contact', 'emergency_phone', 'is_allergic', 'current_session_id', 'bank_name', 'bank_account', 'carnet_url', 'profile_admin', 'profile_opera', 'eye_id'];
