...
// GET /api/staff/live-locations — devuelve ubicaciones activas (últimas 4 horas) para el mapa
...
app.post('/api/admin/user-permissions', async (c) => {
  ...
  const validFields = ['valet_ve', 'valet_mod', 'accesos_ve', 'accesos_mod', 'alquiler_ve', 'alquiler_mod', 'eventos_ve', 'eventos_mod', 'admin_ve', 'admin_mod', 'vip_ve', 'vip_mod', 'seg_ve', 'seg_mod'];
  ...
  await c.env.DB.prepare(`
    ...
    (user_id, valet_ve, valet_mod, accesos_ve, accesos_mod, alquiler_ve, alquiler_mod, eventos_ve, eventos_mod, admin_ve, admin_mod, vip_ve, vip_mod, seg_ve, seg_mod) 
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).bind(user_id, d_valet_ve, d_valet_mod, d_accesos_ve, d_accesos_mod, d_alquiler_ve, d_alquiler_mod, d_eventos_ve, d_eventos_mod, d_admin_ve, d_admin_mod, d_vip_ve, d_vip_mod, d_seg_ve, d_seg_mod)
    .run();
  ...
  try {
    ...
    if (msg && msg.text && msg.text.startsWith('/start')) {
      ...
      if (parts.length > 1 && parts[1].startsWith('link_')) {
        const userId = parts[1].split('_')[1];
        await c.env.DB.prepare('UPDATE users SET telegram_chat_id = ? WHERE id = ?').bind(chatId.toString(), userId).run();
        ...
        await fetch(`https://api.telegram.org/bot${token}/sendMessage`, {
          ...
          body: JSON.stringify({
            chat_id: chatId,
            text: '✅ Cuenta vinculada exitosamente con VIP Eye. Ya puedes enviar reportes y compartir tu ubicación en tiempo real cuando inicies turno.'
          })
        });
      } else {
         await fetch(`https://api.telegram.org/bot${token}/sendMessage`, {
