...
app.get('/api/sessions/active', async (c) => {
  const sessionsRes = await c.env.DB.prepare('SELECT id, name, status, started_at, ended_at, type, client, phone, address, internal_key FROM sessions WHERE status IN ("planning", "active") ORDER BY id ASC').all();
