...
app.get('/api/sessions/active', async (c) => {
  const sessionsRes = await c.env.DB.prepare('SELECT * FROM sessions WHERE status IN ("planning", "active") ORDER BY id DESC').all();
...
app.post('/api/sessions/plan', async (c) => {
  const { name, type, supervisor_id, staff_ids } = await c.req.json();
  ...
  const result = await c.env.DB.prepare('INSERT INTO sessions (name, type, status, supervisor_id) VALUES (?, ?, "planning", ?)')
    .bind(sessionName, sessionType, supervisor_id || null)
    .run();
...
app.post('/api/staff/update', async (c) => {
  ...
  const allowedFields = ['name', 'role', 'pin_hash', 'cedula', 'phone', 'sector', 'emergency_contact', 'emergency_phone', 'is_allergic', 'current_session_id', 'bank_name', 'bank_account'];
