...
app.get('/api/payroll/available-sessions', async (c) => {
  const user = c.get('user');
  const userId = c.req.query('user_id') || user?.id;

  if (!userId) return c.json({ sessions: [] });

  // Seleccionar sesiones activas/planificadas que NO tengan un reporte del usuario
  const query = `
    SELECT * FROM sessions 
    WHERE status IN ("planning", "active") 
    AND id NOT IN (SELECT session_id FROM payroll_submissions WHERE user_id = ?)
    ORDER BY id DESC
  `;
...
app.post('/api/staff/update', async (c) => {
  ...
  const allowedFields = ['name', 'role', 'pin_hash', 'cedula', 'phone', 'sector', 'emergency_contact', 'emergency_phone', 'is_allergic', 'current_session_id'];
...
app.post('/api/payroll/submit', async (c) => {
  ...
  if (!data.session_id || !data.role_at_event || !data.date) {
  ...
  const rateRecord = await c.env.DB.prepare('SELECT rate FROM payroll_rates WHERE role = ?')
    .bind(data.role_at_event.toLowerCase())
    .first<{ rate: number }>();
  
  const amount = rateRecord ? rateRecord.rate : 0;

  // Actualizar datos bancarios en el perfil del usuario para futuras consultas
  ...
  try {
    const staff = await c.env.DB.prepare('SELECT name FROM users WHERE id = ?').bind(data.user_id || user.id).first<any>();
    const session = await c.env.DB.prepare('SELECT name FROM sessions WHERE id = ?').bind(data.session_id).first<any>();
        ...
        html: `
          <div style="font-family:sans-serif; max-width:500px; margin:auto; border:1px solid #eee; border-radius:15px; overflow:hidden; border-top:5px solid #22c55e;">
            <div style="padding:30px;">
              <h2 style="color:#22c55e; margin:0;">NUEVO REPORTE</h2>
              <hr style="border:0; border-top:1px solid #eee; margin:20px 0;">
              <p><strong>EMPLEADO:</strong> ${staff?.name || 'N/A'}</p>
              <p><strong>EVENTO:</strong> ${session?.name || 'N/A'}</p>
              <p><strong>ROL:</strong> ${data.role_at_event}</p>
    ...
    console.error('Error enviando email de reporte:', emailErr);
  }

  return c.json({ success: true, id: result.meta.last_row_id, amount });
});
