...
app.post('/api/sessions/plan', async (c) => {
  const { name, type, supervisor_id, staff_ids, started_at, phone, address, contact_name, email, observations } = await c.req.json();
  ...
  const result = await c.env.DB.prepare(`
    INSERT INTO sessions (name, type, status, supervisor_id, started_at, phone, address, contact_name, email, observations) 
    VALUES (?, ?, "planning", ?, ?, ?, ?, ?, ?, ?)
  `)
    .bind(sessionName, sessionType, supervisor_id || null, started_at || null, phone || null, address || null, contact_name || null, email || null, observations || null)
    .run();
