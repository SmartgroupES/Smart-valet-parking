...
app.post('/api/presupuestos/send-email', async (c) => {
  try {
    const { to, subject, pdfData, filename } = await c.req.json();
    ...
    const resendRes = await fetch('https://api.resend.com/emails', {
        ...
        body: JSON.stringify({
            ...
            to: [to],
            subject: subject,
            html: '<p>Hola,</p><p>Adjunto información referente al presupuesto solicitado.</p><p>Atentamente,<br>Eye Staff / Rentaequipos</p>',
            attachments: [
                {
                    filename: filename,
                    content: pdfData
